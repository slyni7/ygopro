	...
	if (events & BEV_EVENT_CONNECTED) {
		...
		if(create_game) {
			...
			if(bot_mode) {
				...
				cscg.info.duel_rule = (rule == 7) ? 64 : ((rule == 6) ? 50 : rule);
	...
	case STOC_JOIN_GAME: {
		...
		if(pkt->info.duel_rule != DEFAULT_DUEL_RULE) {
			if(pkt->info.duel_rule >= 64)
				myswprintf(msgbuf, L"*%ls\n", dataManager.GetSysString(1266));
			else if(pkt->info.duel_rule >= 50)
				myswprintf(msgbuf, L"*%ls\n", dataManager.GetSysString(1265));
			else
	...
	case MSG_SWAP_GRAVE_DECK: {
		...
		if(mainGame->dInfo.isReplaySkiping) {
			...
			for (auto cit = mainGame->dField.deck[player].begin(); cit != mainGame->dField.deck[player].end(); ) {
				if ((*cit)->type & (TYPE_FUSION | TYPE_SYNCHRO | TYPE_EXTRA | TYPE_XYZ | TYPE_LINK)) {
			...
			for (auto cit = mainGame->dField.deck[player].begin(); cit != mainGame->dField.deck[player].end(); ) {
				...
				if (pcard->type & (TYPE_FUSION | TYPE_SYNCHRO | TYPE_EXTRA | TYPE_XYZ | TYPE_LINK)) {
