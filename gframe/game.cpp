...
bool Game::Initialize() {
	...
	device->setWindowCaption(L"IreinaPro");
	...
	//main menu
	wchar_t strbuf[256];
	myswprintf(strbuf, L"IreinaPro %X.0%X.%X", PRO_VERSION >> 12, (PRO_VERSION >> 4) & 0xff, PRO_VERSION & 0xf);
...
//	btnTestMode = env->addButton(rect<s32>(10, 135, 270, 165), wMainMenu, BUTTON_TEST_MODE, dataManager.GetSysString(1203));
	...
	cbDuelRule->addItem(dataManager.GetSysString(1265));
	cbDuelRule->addItem(dataManager.GetSysString(1266));
...
void Game::MainLoop() {
	...
	while(device->run()) {
		...
		if(cur_time >= 1000) {
			myswprintf(cap, L"IreinaPro FPS: %d", fps);
...
#ifdef _WIN32
			const wchar_t* fname = irr::core::stringw(archive->getFullFileName(j)).c_str();
#else
			wchar_t fname[1024];
...
void Game::ShowCardInfo(int code, bool resize) {
	...
	if(!gameConf.hide_setname) {
		...
		if(sc || dataManager.CheckFormatSetName(code)) {
			offset = 23;// *yScale;
			myswprintf(formatBuffer, L"%ls%ls", dataManager.GetSysString(1329), dataManager.FormatSetName(sc, code));
